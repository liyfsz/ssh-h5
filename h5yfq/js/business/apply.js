...
getSessionId();
   ...
   $("form").find("button").live("click",function(ev){ 
   	var ajaxUrl = baseUrl+"/seller/audit/appCreate";
    if($("#company").val()==''){
          showInfo("请输入公司名称");
   ...
   /* }else if($("#email").val() == ''){
      ...
      showInfo("对公银行账号格式错误");
    	...
    	if($(ev.target).attr("id") == 'updateInfor')
   	    ajaxUrl = baseUrl+"/appUpdateMySeller";
        ...
        $.ajax({
          ...
          success: function(data) {
            if (data.resultCode =="SUCCESS") {
              var upLoadPic = {
                "backType":"D",
                "backUrl":"resources/h5/pages/business/success.html",
                "pics":[
                  {
                    "TakeOrChoose":"TC",
                    "picName":"请上传营业执照扫描件",
                    "picType":"up_bussinessLicenseImage",
                    "req":"1"
                    },
                  {
                    "TakeOrChoose":"TC",
                    ...
                    },
                  {
                    "TakeOrChoose":"TC",
                    "picName":"请上传身份证正面照片",
                    "picType":"up_personCardUp",
                    "req":"1"
                    },
                  {
                    "TakeOrChoose":"TC",
                    "picName":"请上传身份证反面照片",
                    "picType":"up_personCardDown",
                    "req":"1"
                    },
                  {
                    "TakeOrChoose":"TC",
                    "picName":"请上传开户许可证",
                    "picType":"up_accountImg",
                    "req":"0"
                    }
                  ],
                  "turnoverNumber":GetQueryString('sessionId'),
                  "url":baseUrl+"/seller/audit/appUpdatePhoto"
                  };
                var srtJson = JSON.stringify(upLoadPic);
                if(getTerminal()){
                   window.android.takePic(srtJson);
                }else if(getTerminal() == false){
                  window.webkit.messageHandlers.takePic.postMessage(upLoadPic);
                }
            }else if(data.errorCode =="ERROR_NOAUTH"){
            	getSessionId();
            }else if(data.errorCode =="ERROR_SYSTEM"){
              showInfo('您已经申请过了，请不要重复申请')
            }else if(data.errorCode =="ERROR_REGREFEREE"){
            	showMask();
            	$(".mask").find(".inform").html(data.message);
            }
          }
        });
        }
...
$('#bankNameBranch,#bankNameBranchPr').live("keyup",function(event){
    $.ajax({
